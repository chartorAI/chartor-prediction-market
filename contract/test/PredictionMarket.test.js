...
describe("PredictionMarket", function () {
  ...
  describe("Platform Fee Collection", function () {
    ...
    it("should collect 1.5% platform fee on YES share purchases", async function () {
      ...
      const cost = await predictionMarket.calculateYesCost(
        marketId,
        shareAmount
      );
      ...
      expect(await predictionMarket.getPlatformFees(marketId)).to.equal(
        expectedFee
      );
      expect(await predictionMarket.getMarketBalance(marketId)).to.equal(
        expectedMarketFunds
      );
      expect(await predictionMarket.getTotalPlatformFees()).to.equal(
        expectedFee
      );
    });
    ...
    it("should collect 1.5% platform fee on NO share purchases", async function () {
      const shareAmount = ethers.parseEther("1");
      const cost = await predictionMarket.calculateNoCost(
        marketId,
        shareAmount
      );
      const expectedFee = (cost * BigInt(PLATFORM_FEE_RATE)) / 10000n;
    ...
    it("should accumulate platform fees across multiple trades", async function () {
      ...
      // Second trade
      const noCost = await predictionMarket.calculateNoCost(
        marketId,
        shareAmount
      );
      await predictionMarket
  ...
  describe("Fund Isolation Between Markets", function () {
    ...
    it("should track market balances separately", async function () {
      ...
      // Trade in market 1
      const cost1 = await predictionMarket.calculateYesCost(
        marketId1,
        shareAmount
      );
      await predictionMarket
      ...
      // Trade in market 2
      const cost2 = await predictionMarket.calculateYesCost(
        marketId2,
        shareAmount
      );
      await predictionMarket
      ...
      const expectedBalance1 =
        cost1 - (cost1 * BigInt(PLATFORM_FEE_RATE)) / 10000n;
      const expectedBalance2 =
        cost2 - (cost2 * BigInt(PLATFORM_FEE_RATE)) / 10000n;

      expect(await predictionMarket.getMarketBalance(marketId1)).to.equal(
        expectedBalance1
      );
      expect(await predictionMarket.getMarketBalance(marketId2)).to.equal(
        expectedBalance2
      );
    });
  ...
  describe("Proportional LMSR Payouts", function () {
    ...
    it("should calculate proportional payouts correctly", async function () {
      ...
      expect(
        await predictionMarket.getTraderPayout(marketId, trader1.address)
      ).to.equal(0n);
      expect(
        await predictionMarket.getTraderPayout(marketId, trader2.address)
      ).to.equal(0n);
      expect(
        await predictionMarket.getTraderPayout(marketId, trader3.address)
      ).to.equal(0n);

      // Verify the proportional calculation logic by checking market balance and shares
      const totalMarketBalance =
        await predictionMarket.getMarketBalance(marketId);
      ...
      const expectedPayout1 =
        (shareAmount1 * totalMarketBalance) / totalYesShares; // 1/3 of market
      const expectedPayout2 =
        (shareAmount2 * totalMarketBalance) / totalYesShares; // 2/3 of market
    ...
    it("should ensure total payouts equal market balance", async function () {
      ...
      const totalPayoutBeforeResolution =
        await predictionMarket.getTotalPayout(marketId);
  ...
  describe("Emergency Admin Functions", function () {
    ...
    it("should allow owner to emergency withdraw all funds", async function () {
      ...
      expect(ownerBalanceAfter).to.equal(expectedBalance);
      expect(
        await ethers.provider.getBalance(predictionMarket.target)
      ).to.equal(0n);
    });
  ...
  describe("Performance Optimization - Winning Shares Cache", function () {
    ...
    it("should initialize cache to zero for new markets", async function () {
      expect(await predictionMarket.totalWinningSharesCache(marketId)).to.equal(
        0n
      );
    });
    ...
    it("should cache total winning shares during payout distribution", async function () {
      ...
      // Add some trades
      const yesCost1 = await predictionMarket.calculateYesCost(
        marketId,
        shareAmount1
      );
      await predictionMarket
    ...
    it("should maintain cache consistency across multiple queries", async function () {
      ...
      const payout1_call1 = await predictionMarket.getTraderPayout(
        marketId,
        trader1.address
      );
      const payout1_call2 = await predictionMarket.getTraderPayout(
        marketId,
        trader1.address
      );
      const payout2_call1 = await predictionMarket.getTraderPayout(
        marketId,
        trader2.address
      );
      const payout2_call2 = await predictionMarket.getTraderPayout(
        marketId,
        trader2.address
      );
    ...
    it("should demonstrate gas efficiency improvement", async function () {
      ...
      const payout1 = await predictionMarket.getTraderPayout(
        marketId,
        trader1.address
      );
      const payout2 = await predictionMarket.getTraderPayout(
        marketId,
        trader2.address
      );
      const payout3 = await predictionMarket.getTraderPayout(
        marketId,
        trader3.address
      );
